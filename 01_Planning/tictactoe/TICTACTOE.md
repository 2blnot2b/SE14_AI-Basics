...
- ### Problem Description:
    ...
    1. **board** $\rightarrow$ Representation of each **state**

    2. **initial_state(board)** $\rightarrow$ The **starting state** of the board (9 possible moves for X)
        
        ![Initial-State](../../Assets/planning/Initial-State.png)

    3. **player_move(board)** $\rightarrow$ Deciding **player's turn** based on the board progression (MIN or MAX)
        
        ![Player-Func](../../Assets/planning/Player-Func.png)

    4. **moves(board)** $\rightarrow$ A set of legal **moves** on the board
        
        ![Initial-State](../../Assets/planning/Moves-Func.png)

    5. **result(board, move)** $\rightarrow$ A **transition model** which defines the state resulting from **choosing a move** on the board
        
        ![Result-Func](../../Assets/planning/Result-Func.png)

    6. **winner(board)** $\rightarrow$ A set of rules that define a **winner** needs 3 **X** or 3 **O** **consequtively** in a **horizontal**, **vertical**, or **diagonal** line, otherwise return none
        
        ![Result-Func](../../Assets/planning/Winner-Func.png)

    7. **terminal(board)** $\rightarrow$ A test which is **true** when the game is **over** and **false** if **otherwise**. The game ends with the so called **terminal state**

        ![Result-Func](../../Assets/planning/Terminal-Func.png)

    8. **utility(board)** $\rightarrow$ Also called **objective function** or **payoff function**, is an assignment of the final numeric value to the player when the game is in the terminal state
...
- #### **Minimax**
    ...
    **"Minimizing the possible loss for a worst case scenarion which is the maximum loss."**
    ...
    If the maximum depth of the tree is $n$ and there are $b$ legal moves in each turn then the time complexity is measured by $O(b^n)$ and the space complexity is $O(bn)$. Mathematically, minimax algorithm is written as

    $$
    \bar{v_i}=\min_{a-i}\max_{a_i}v_i(a_i,a_-i)
    $$

    In Tic-Tac-Toe, each state of the board is presented in each leaf node of the entire tree. In each turn, MAX will iterate over all possible states which are valued numerically as a sign to show that one state is better than the other states. Max choses the highest value possible after comparing it to a helper value which is $-\infin$ or the worst possible value for MAX. MIN does exactly the same steps but unlike MAX, MIN choses the lowest possible value after comparing it to its worst possible value which is $\infin$.
...
- #### **Alpha-Beta Pruning**
    ...
    - $\alpha$ represents the value (best) of the best choice found so far at any node along the path of MAX; Also called "At least" with the starting value of $-\infin$.
    - $\beta$ represents the value (lowest) of the best choice found so far at any node along the path of MIN; Also called "At most" with the starting value of $\infin$.

    Time complexity in alpha-beta pruning with even is $O(b^{n/2})$ and so the space complexity is $O(bn/2)$. This means that alpha-beta with perfect ordering can solve a tree approximately twice as deep as minimax in the same amount of time.
